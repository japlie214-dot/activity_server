# tools/stock_financials/resume.py
from tools.base import BaseResumeHandler, ResumeReport
from database.connection import DatabaseManager

class StockFinancialsResumeHandler(BaseResumeHandler):
    def check_resume_state(self) -> ResumeReport:
        conn = DatabaseManager.get_read_connection()
        cursor = conn.execute("SELECT status, COUNT(*) as cnt FROM job_items WHERE job_id = ? GROUP BY status", (self.job_id,))
        ...
        return ResumeReport(
            ...
            message=f"Extraction interrupted. {completed} complete, {pending_failed} pending.",
        )
