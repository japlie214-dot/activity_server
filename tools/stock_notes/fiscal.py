...
"""Fiscal quarter utilities for stock_notes. All fiscal year-end detection is dynamic."""
...
def _detect_fy_month(ticker: str, company=None) -> int:
    ...
    # Tier 1: company.fiscal_year_end
    try:
        if company is None:
        ...
        if filings and len(filings) > 0:
            ...
            if hasattr(obj, "financials") and obj.financials is not None:
                ...
                if hasattr(fin, "xb") and fin.xb is not None:
                    ...
                    if fy_month_xbrl and isinstance(fy_month_xbrl, int) and 1 <= fy_month_xbrl <= 12:
                        return fy_month_xbrl
    except Exception:
        pass

    # Tier 3: Heuristic from most common month in 10-K filings
    try:
        if company is None:
            company = Company(ticker)
        filings = company.get_filings(form="10-K", amendments=False)
        if filings:
            ...
            for f in list(filings)[:5]:
                ...
                if period:
                    try:
                        months.append(datetime.strptime(str(period), "%Y-%m-%d").date().month)
                    except ValueError:
                        pass
            if months:
    ...
    log.warning(f"StockNotes:Fiscal:Fallback — defaulting to 12 for {ticker}")
    return 12
...
def quarter_date_range(start_date: str, end_date: str, fiscal_year_end_month: int = 12,
                       max_quarters: int = 12) -> List[Tuple[int, int]]:
